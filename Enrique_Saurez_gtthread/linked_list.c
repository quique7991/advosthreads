...
int pop_node_gtthread(gtthread_t thread){
	...
	else if(thread == current->id){/*Equivalent to gtthread_exit with a value of -1*/
	  deactivate_signal();
	  /*Push results so other threads can be the retval return*/
	  copy_result(current->id,(void*) -1);
...
int getValue(gtthread_t thread, void **ret,gtthread_t myself){
	...
	if(join_results[thread].is_running){
		if(join_results[thread].waiting_to_join==-1){
			join_results[thread].waiting_to_join=myself;
		}
		else{/*Thread is already joining another thread*/
			if(join_results[join_results[thread].waiting_to_join].is_running){
				if(myself!=join_results[thread].waiting_to_join){
					//Undefined behavior, we choose to return the result, but we don't detect if there is a deadlock
				}
			}
			else{/*Thread was canceled or finished without the join*/
				join_results[thread].waiting_to_join = myself;
			}
		...
		else if((join_results[myself].waiting_to_join==thread)){		/*If the threads are waiting for each other */
			return 35; /*EDEADLK occurs*/
		}
	}
	else{
		return -1; /*value does not exists yet;*/
	}
	return result;
}
